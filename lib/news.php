	...
	foreach ( $items as $item ) {
		...
		if ( $item->post_type === 'post' && array_key_exists( 'is_event', $custom ) ) {
			if ( $custom['is_event'][0] === '1' ) {
				...
				$eventDate = '<div class="date-event"><img alt="calendar icon" src="<?php echo get_template_directory_uri(); ?>/images/calendar.svg" width="13px" height="13px" ><span class="event">' . date_format( $eventDate, 'F j' ) . '</span>';
