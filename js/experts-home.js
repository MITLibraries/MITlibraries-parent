...
function parseExperts(data) {
	...
	jQuery('.expert').append('<a class="link-profile no-underline">');
	// Append expert image div
	jQuery('.expert .link-profile').append('<img alt="" class="expert-photo">');
	// Append empty spans for expert names
	jQuery('.expert .link-profile').append('<span class="name"></span>');
	// Append empty spans for expert titles
	jQuery('.expert .link-profile').append('<span class="title-job"></span>');
	// Add expert name to appropriate span
	jQuery('.expert .name:eq(0)').text(expertName1);
	jQuery('.expert .name:eq(1)').text(expertName2);
	jQuery('.expert .name:eq(2)').text(expertName3);
	jQuery('.expert .name:eq(3)').text(expertName4);
	// Add image URL to src attribute
	jQuery('.expert .expert-photo:eq(0)').attr('src', expertPhoto1);
	jQuery('.expert .expert-photo:eq(1)').attr('src', expertPhoto2);
	jQuery('.expert .expert-photo:eq(2)').attr('src', expertPhoto3);
	jQuery('.expert .expert-photo:eq(3)').attr('src', expertPhoto4);
	// Add expert excerpt
	jQuery('.expert .title-job:eq(0)').html(expertExcerpt1);
	jQuery('.expert .title-job:eq(1)').html(expertExcerpt2);
	jQuery('.expert .title-job:eq(2)').html(expertExcerpt3);
	jQuery('.expert .title-job:eq(3)').html(expertExcerpt4);
	// Add expert URL
	jQuery('.expert .link-profile:eq(0)').attr('href', expertURL1);
	jQuery('.expert .link-profile:eq(1)').attr('href', expertURL2);
	jQuery('.expert .link-profile:eq(2)').attr('href', expertURL3);
	jQuery('.expert .link-profile:eq(3)').attr('href', expertURL4);
	// Add the expert count to the "All Experts" button
	jQuery('.view-experts .count').text(data.length);
}
...
jQuery(function(){
	...
	jQuery.getJSON('/wp-json/wp/v2/experts?per_page=99')
		.done(function(data){
