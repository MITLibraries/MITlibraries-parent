...
function filterAlerts(posts) {
	...
	for (i = 0; i < posts.length; i++) {
		...
		if (jQuery(post.meta).length) {
			...
			if (jQuery(post_meta.alert).length && true === post_meta.alert && true === post_meta.confirm_alert ) {
				filtered.push(post);
			}
		}
	};
...
function renderAlert(markup,id) {
	...
	if (Modernizr.localstorage) {
		...
		if (localStorage.getItem('alert_closed-' + id) !== 'true') {
			// Append the template
			jQuery(markup).prependTo('.wrap-page');
			jQuery('.gldp-default').animate({"top":"292px"});
			// Remove the necessary transition class with a timeout, so that the animation shows.
			setTimeout(function() {
				jQuery('.posts--preview--alerts').removeClass('transition-vertical--hide');
			}, 300);
		}
...
function setClosable(alert_ID) {
	...
	jQuery('.posts--preview--alerts .post').append('<a href="#0" id="close" class="action-close"><span class="sr">Dismiss</span><i class="icon-remove-sign" aria-hidden="true"></i></a>');
	// On click
	jQuery('#close').click(function(){
		// Add the necessary transition hide class
		jQuery('.posts--preview--alerts').addClass('transition-vertical--hide');
		jQuery('.gldp-default').css({"top":"105px"});
		// If localStorage
		if (Modernizr.localstorage) {
